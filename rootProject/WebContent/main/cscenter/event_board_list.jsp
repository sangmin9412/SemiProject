			...
			<div class="board-list">
				<table>
					...
					<tbody>
						...
						<c:if test="${ !empty eventList }">
						<c:forEach items="${ eventList }" var="dto" varStatus="cnt">
					    <tr>
							<td scope="row">${ ((page-1) * limit) + cnt.count }</td>
							<td class="subject">&nbsp;
								<a href="eventView.eb?boardNum=${ dto.boardNum }" >${ dto.boardSubject }</a>
							</td>
							<td class="date"><fmt:formatDate value="${ dto.startDate }" pattern="yyyy-MM-dd" /></td>
							<td class="date"><fmt:formatDate value="${ dto.endDate }" pattern="yyyy-MM-dd" /></td>
							<td class="cdt">
								<c:if test="${ cDate >= dto.endDate }">
									마감
								</c:if>
								<c:if test="${ cDate < dto.endDate && cDate > dto.startDate }">
									진행
								</c:if>
								<c:if test="${ cDate < dto.startDate }">
									대기
								</c:if>
							</td>
