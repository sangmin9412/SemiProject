...
public class AnswerModifyAction {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		int conNum = 0;
		String realPath = request.getServletContext().getRealPath("/main/cscenter/answerupload/");
		...
		try {
			if (conNum >= 1) {
				response.sendRedirect("answerView.ab?boardNum=" + boardNum);
			} else {
				...
				out.println("location.href = 'answerModify.ab?boardNum="+boardNum+"'");
				out.println("</script>");
				out.close();		
			}
		} catch (Exception e) {
